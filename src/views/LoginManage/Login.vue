...
<template>
	<div class="login-container" style="background-image: url(../../images/login_background.png);">
		<el-form ref="loginForm" :model="loginForm" :rules="loginRules" class="login-form" auto-complete="on" label-position="left">
			...
			<div v-if="isShowCodeLogin">
				<el-form-item prop="username">
					...
					<el-input ref="username" v-model="loginCodeForm.phone" placeholder="请输入你的手机号" name="phone" type="text" tabindex="1"
					 auto-complete="on" />
				</el-form-item>
		...
		<div style="position: fixed; bottom: 0px;">
			<el-alert title="使用系统前提介绍" type="info" close-text="知道了,前往下载" @close="gotoDownLoadChrome" description="为了提高用户体验,使用本系统请尽量使用谷歌浏览器(Chrome)"></el-alert>
		</div>
	</div>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 登录
			handleLogin() {
				if (this.loginForm.username.replace(/\s/g, "").length >= 20) {
					this.$message({
						showClose: true,
						message: "密码长度在6-20位内",
						type: "warning"
					});
				} else if (this.loginForm.password.replace(/\s/g, "").length >= 11) {
					this.$message({
						showClose: true,
						message: "用户名长度在11位内",
						type: "warning"
