...
<template>
	<div id="productInfoDiv" v-loading="isProductSPULoading" style="margin:30px ;">
		...
		<el-form :inline="true" class="demo-form-inline productSearchForm" label-width="70px">
			<el-row :gutter="24">
				<el-col :lg="{span: 6}" :xs="{span: 24}" :sm="{span: 24}">
					<el-form-item label="商品状态" style="width: 100% !important;">
						<el-select v-model="productPage.state" placeholder="选择商品状态" style="width: 100% !important;">
							<el-option :label="state.key" :value="state.value" v-for="state in productState" :key="state.value" />
						</el-select>
					</el-form-item>
				</el-col>
				...
				<el-col :lg="{span: 6}" :xs="{span: 24}" :sm="{span: 24}">
					<el-form-item label="搜索商品" style="width: 100%;">
						<el-input v-model="productPage.productName" placeholder="商品的名称"><i slot="prefix" class="el-icon-edit" /></el-input>
					</el-form-item>
				</el-col>
			...
			<el-row :gutter="24">
				<el-col :lg="{span: 6}" :xs="{span: 24}" :sm="{span: 24}">
					<el-button class="searchButton" type="primary" icon="el-icon-search" style="width:  100%;" @click="queryProductInfo()">搜索</el-button>
				</el-col>
			</el-row>
		</el-form>
			...
			<el-col :sm="{span: 24}" :xs="{span: 24}">
				...
				<el-table v-loading="listLoading" class="title-menu-min" :data="prodcutData" border style="width: 100%">
					...
					<el-table-column prop="city" label="商品类目" width="90">
						<el-button slot-scope="scope" type="text" size="small" @click="showProductLeimu(scope.row.categoryName)">查看详情</el-button>
					</el-table-column>
					<el-table-column prop="totalKc" label="商品总库存" width="150" />
					<el-table-column label="商品参数" width="100">
						<el-button slot-scope="scope" type="text" size="small" @click="shwoProductSpuInfo(scope.row.productId)">查看详情</el-button>
					</el-table-column>
					<el-table-column label="SKU参数" width="100">
						<el-button slot-scope="scope" type="text" size="small" @click="showProductSkuInfo(scope.row.productId)">查看详情</el-button>
					</el-table-column>
					<el-table-column label="商品状态" width="100">
						<el-link type="primary" slot-scope="scope" :underline="false" v-if="scope.row.productState == 1">上架</el-link>
						<el-link type="danger" slot-scope="scope" :underline="false" v-else-if="scope.row.productState == 2">下架</el-link>
						<el-link type="danger" slot-scope="scope" :underline="false" v-else-if="scope.row.productState == 3">删除</el-link>
						<el-link type="warning" slot-scope="scope" @click="editZhongGoToShangJia(scope.$index , scope.row.productId)"
						 :underline="false" v-else-if="scope.row.productState == 4">编辑中</el-link>
						<el-link type="warning" slot-scope="scope" :underline="false" v-else-if="scope.row.productState == 5">审核中</el-link>
						<el-link type="primary" slot-scope="scope" :underline="false" v-else-if="scope.row.productState == 6">库存充足</el-link>
						<el-link type="warning" slot-scope="scope" :underline="false" v-else-if="scope.row.productState == 7">库存不足</el-link>
						<el-link type="danger" slot-scope="scope" :underline="false" v-else-if="scope.row.productState == 8">审核不通过</el-link>
						<el-link type="danger" slot-scope="scope" :underline="false" v-else>未知状态</el-link>
					</el-table-column>
					...
					<el-table-column fixed="right" label="操作" width="150">
						<template slot-scope="scope">
							...
							<el-button type="text" size="small" @click="delProduct(scope.$index,scope.row)">删除</el-button>
						</template>
					</el-table-column>
				</el-table>
			</el-col>
		...
		<el-row :gutter="24">
			<el-col :sm="{span: 24}" :xs="{span: 24}">
				<el-pagination @size-change="productCurrentSize" :small="pageFlag" :pager-count="5" @current-change="productCurrentPage"
				 :current-page="productPage.currentPage" :page-sizes="[1,2,3,5,10]" hide-on-single-page='true' :page-size="productPage.currentSize"
				 layout="total, sizes, prev, pager, next, jumper" :total="productPage.totalSize">
				</el-pagination>
		...
		<!-- ======================= 添加商品弹出层 =========================  -->
		<el-dialog v-el-drag-dialog top="7vh" width="60%" :append-to-body="true" custom-class="addProductDiv" :title="addProductTitle"
		 :visible.sync="addProductFlag" class="title-menu-min">
			<!-- ======================= 步骤一=========================  -->
					...
					<el-row :gutter="24">
						<el-col :sm="{span: 11,offset: 1}" :xs="{span: 23}">
							<el-form-item label="是否支持" style="width:100%;" class="addProductItem">
								...
								<el-input type="number" v-if="retreatRadio == 1" v-model="addProductTwoFrom.goodsDatanum" placeholder="设置无理由退款天数(默认7天)"
				...
				<el-form ref="addProductTwoFrom" label-position="left" :inline="true" :model="addProductTwoFrom" label-width="80px">
					<el-row v-for="(o,index) in addProductSkuForm.domains" :key="o.key" class="spusRowClass" :gutter="24">
						...
						<el-card shadow="always" class="SpusClass" style="margin-bottom: 5px;width:95%;">
							<el-col :sm="{span: 8}" :xs="{span: 12}">
								<el-form-item>
									<el-input v-if="o.isSystemCreate != 1" v-model="o.productSkuKeyName" class="SpuInput" maxlength="10"
									 placeholder="SKU名称" />
		...
		<el-dialog v-el-drag-dialog title="商品类目信息" width="400px" :visible.sync="productLeiMuInfoFlag" class="addProduct"
		 @close="spuWindow">
			<div>
		...
		<!-- ======================= 查看商品SPU =========================  -->
		<el-dialog v-el-drag-dialog v-loading="isProductInfoLoading" title="商品SPU信息" top="7vh" :modal-append-to-body="true"
		 :visible.sync="productSpuFlag" custom-class="editSpuDiv" width="70%">
			<div v-if="productSpuFlag2">
				<div class="title-menu-min spuDialogClass" style="max-height: 400px;">
					<el-table ref="multipleTable" :data="productSpuInfo" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange">
						...
						<el-table-column label="状态" width="80" show-overflow-tooltip>
							<template slot-scope="scope">
								...
								<el-tag title="恢复正常使用" v-else-if="scope.row.productSpuState == 2" @click="normalSpu(scope.$index,scope.row)"
								 type="danger" style="cursor: pointer;">被删除</el-tag>
								<el-tag title="恢复正常使用" v-else type="warning" @click="normalSpu(scope.$index,scope.row)" style="cursor: pointer;">注销中</el-tag>
							</template>
						</el-table-column>
						<el-table-column label="操作" fixed="right" width="100">
							<el-button type="text" slot-scope="scope" size="small" @click="addProductInfoSpu2(scope.$index,scope.row)">编辑</el-button>
				...
				<el-row :gutter="24">
					...
					<el-col :sm="{span: 8}" :xs="{span: 23}">
						<el-button style="margin-top: 12px;width: 100%;" type="primary" @click="batchCancelSpu()">批量注销SPU</el-button>
					</el-col>
					<el-col :sm="{span: 8}" :xs="{span: 23}">
				...
				<el-form ref="addProductTwoFrom" label-position="left" :inline="true" :model="addProductTwoFrom" label-width="80px">
					<el-row v-for="(o,index) in addProductSpuForm" :key="o.key" class="spusRowClass" :gutter="24">
						...
						<el-card shadow="always" class="SpusClass" style="margin-bottom: 5px;width:95%;">
							...
							<el-col :sm="{span: 8}" :xs="{span: 12}">
								<el-form-item>
									<el-input type="textarea" maxlength="20" show-word-limit v-model="o.productSpuName" class="SpuInput"
							...
							<el-col :sm="{span: 8}" :xs="{span: 12}">
								<el-form-item>
									<el-input type="textarea" maxlength="100" show-word-limit v-model="o.productSpuValue" class="SpuInput"
							...
							<el-col :sm="{span: 4}" :xs="{span: 12}">
								<el-form-item>
									<el-input v-model="o.productSpuOrder" maxlength="11" oninput="value=value.replace(/[^\d]/g,'')"
									 show-word-limit class="SpuInput2" placeholder="排序" />
								</el-form-item>
							</el-col>
					...
					<el-card shadow="always" class="SpusClass" style="margin-bottom: 5px;width:95%;">
						...
						<el-collapse accordion>
							<el-collapse-item title="操作菜单" name="1">
								...
								<el-col :sm="{span: 8}" :xs="{span: 23}">
									<el-button style="width: 100%;" type="primary" @click="addProductSpuInfo" class="addAndClose">保存当前配置</el-button>
								</el-col>
								<el-col :sm="{span: 8}" :xs="{span: 23}">
				...
				<el-form ref="addProductTwoFrom" label-position="left" :inline="true" :model="addProductTwoFrom" label-width="80px">
					<el-row class="spusRowClass" :gutter="24">
						...
						<el-card shadow="always" class="SpusClass" style="margin-bottom: 5px;width:95%;">
							<el-col :sm="{span: 8}" :xs="{span: 12}">
								<el-form-item>
									<el-input type="textarea" maxlength="20" show-word-limit v-model="currentEditSpuInfo.productSpuName" class="SpuInput"
									 placeholder="请输入商品参数名称" />
								</el-form-item>
							</el-col>
							...
							<el-col :sm="{span: 8}" :xs="{span: 12}">
								<el-form-item>
									<el-input type="textarea" maxlength="100" show-word-limit v-model="currentEditSpuInfo.productSpuValue" class="SpuInput"
							...
							<el-col :sm="{span: 4}" :xs="{span: 12}">
								<el-form-item>
									<el-input v-model="currentEditSpuInfo.productSpuOrder" oninput="value=value.replace(/[^\d]/g,'')" maxlength="11"
									 show-word-limit class="SpuInput2" placeholder="排序" />
								</el-form-item>
							</el-col>

							<el-col :sm="{span: 4}" :xs="{span: 12}">
								<el-button type="danger" icon="el-icon-delete" v-if="currentEditSpuInfo.productSpuState != 2" circle
								 @click.pprevent="removeProducSpu(currentEditSpuInfo)" />
								<el-button type="danger" icon="el-icon-delete" v-else circle disabled="disabled" />
							</el-col>
						</el-card>
					</el-row>
				</el-form>
		...
		<el-dialog v-el-drag-dialog v-loading="isProductSPULoading" title="商品信息" :modal-append-to-body="false" :lock-scroll="true"
		 :visible.sync="productInfoFlag" width="70%" custom-class="addProductDiv" :top="productTopHtml">
			...
			<!-- ======================= 评论详细信息 =========================  -->
			<el-dialog v-el-drag-dialog width="50%" custom-class="addProductDiv" :top="productTopHtml" title="商品评价信息"
			 :visible.sync="productInfoFlag3" append-to-body>
		...
		<!-- ======================= 商品SKU信息管理 =========================  -->
		<el-dialog v-el-drag-dialog v-loading="isProductSKULoading" width="77%" :top="productTopHtml" custom-class="addProductDiv"
		 title="商品SKU信息" :visible.sync="productInfoSkuInfos" append-to-body>
			...
			<div style="max-height: 550px;" class="title-menu-min">
				<el-table :data="SKUInfoForm" border style="width: 100%">
					...
					<el-table-column label="库存数量" width="120">
						<el-input oninput="value=value.replace(/[^\d]/g,'')" maxlength="11" show-word-limit v-model="scope.row.productSkuKc"
						 slot-scope="scope" :value="scope.row.productSkuKc" />
					</el-table-column>
					<el-table-column prop="province" label="价格" width="120">
						<el-input v-model="scope.row.productSkuPrice" slot-scope="scope" maxlength="9" show-word-limit :value="scope.row.productSkuPrice" />
					</el-table-column>
				</el-table>
		...
		<el-dialog v-el-drag-dialog v-loading="editProductFlag" width="77%" :top="productTopHtml" custom-class="productSkuInfoDialog title-menu-min"
		 title="編輯商品" :visible.sync="editProductInfoFlag" append-to-body>
				...
				<el-form ref="editProduct" :model="editProduct" label-width="80px">
					...
					<el-row :gutter="24">
						...
						<el-col :sm="{span: 11,offset: 1}" :xs="{span: 23}">
							<el-form-item class="addProductItem" label="商品名称" style="width:100%;">
								<el-input maxlength="200" show-word-limit v-model="editProductForm.productName" class="addProductFormInput"
								 placeholder="请输入商品名称" />
							</el-form-item>
						</el-col>
						...
						<el-col :sm="{span: 11,offset: 1}" :xs="{span: 23}">
							...
							<el-form-item label="商品品牌" class="addProductItem" style="width:100%;">
								<el-select v-model="editProductForm.productBrandId" style="width:100%;" class="addProductFormInput" placeholder="请选择商品品牌">
									<el-option :label="item.brandName" v-for="(item, index) in brands" :key="index" :value="item.brandId" />
								</el-select>
							</el-form-item>
						</el-col>
					...
					<el-row :gutter="24">
						<el-col :sm="{span: 11,offset: 1}" :xs="{span: 23}">
							<el-form-item class="addProductItem" label="市场价格" style="width:100%;">
								<el-input maxlength="10" show-word-limit v-model="editProductForm.productBazaarPrice" class="addProductFormInput"
								 placeholder="请输入市场价格" />
							</el-form-item>
						</el-col>
						...
						<el-col :sm="{span: 11,offset: 1}" :xs="{span: 23}">
							...
							<el-form-item label="商城价格" class="addProductItem">
								<el-input maxlength="10" show-word-limit v-model="editProductForm.productSystemPrice" class="addProductFormInput"
								 placeholder="请输入商城价格" />
								</el-select>
							</el-form-item>
						</el-col>
					...
					<el-row :gutter="24">
						<el-col :sm="{span: 11,offset: 1}" :xs="{span: 23}">
							<el-form-item class="addProductItem" label="商品卖点" style="width:100%;">
								<el-input maxlength="100" show-word-limit v-model="editProductForm.productPoints" class="addProductFormInput"
								 placeholder="请输入商品卖点" />
							</el-form-item>
						</el-col>
						...
						<el-col :sm="{span: 11,offset: 1}" :xs="{span: 23}">
							...
							<el-form-item label="排序" class="addProductItem">
								<el-input maxlength="11" show-word-limit v-model="editProductForm.productOrderRule" class="addProductFormInput"
								 placeholder="请输入排序" />
								</el-select>
							</el-form-item>
						</el-col>
					...
					<el-row :gutter="24">
						<el-col :sm="{span: 11,offset: 1}" :xs="{span: 23}">
							<el-form-item class="addProductItem" label="商品单位" style="width:100%;">
								<el-input maxlength="10" show-word-limit v-model="editProductForm.productUnit" class="addProductFormInput"
								 placeholder="请输入商品单位" />
							</el-form-item>
						</el-col>
						...
						<el-col :sm="{span: 11,offset: 1}" :xs="{span: 23}">
							...
							<el-form-item label="商品简述" class="addProductItem">
								<el-input type="textarea" maxlength="1000" show-word-limit v-model="editProductForm.productSketchContentl"
								 class="addProductFormInput" placeholder="请输入商品简述" />
								</el-select>
							</el-form-item>
						</el-col>
					</el-row>
					...
					<el-row :gutter="24">
						<el-col :sm="{span: 11,offset: 1}" :xs="{span: 23}">
							<el-form-item label="是否支持" style="width:100%;" class="addProductItem">
								...
								<el-input oninput="value=value.replace(/[^\d]/g,'')" maxlength="3" show-word-limit v-if="editProductForm.productIfSupport == 1"
								 v-model="editProductForm.productSupportDay" placeholder="设置无理由退款天数(默认7天)" class="addProductFormInput" />
							</el-form-item>
						</el-col>
					</el-row>
					...
					<el-row :gutter="24">
						<el-col :sm="{span: 11,offset: 1}" :xs="{span: 23}">
							<el-form-item class="addProductItem" label="商品图片" style="width:100%;">
								<el-button type="primary" style="width:270px;" @click="editProductImageFun" :disabled="!isEditFlag">修改商品图片</el-button>
							</el-form-item>
						</el-col>
						...
						<el-col :sm="{span: 11,offset: 1}" :xs="{span: 23}">
							...
							<el-form-item label="商品详情" class="addProductItem">
								<el-button type="primary" style="width:270px;" @click="addProductInfos" :disabled="!isEditFlag">修改商品详情</el-button>
							</el-form-item>
						</el-col>
					</el-row>
					...
					<el-row class="spusRowClass" :gutter="24">
						...
						<el-col :sm="{span: 6,offset: 1}" :xs="{span: 24}">
							<el-button type="primary" style="margin-top: 12px;  width: 100% !important; " @click="editProductFun()"
							 :disabled="!isEditFlag">保存编辑</el-button>
						</el-col>
			...
			<!-- ======================= 修改商品图片 =========================  -->
			<el-dialog v-el-drag-dialog width="77%" :top="productTopHtml" custom-class="productSkuInfoDialog title-menu-min"
			 title="修改商品图片" :visible.sync="editProductImage" append-to-body>
				...
				<el-table :data="productImages" style="width: 100%" max-height="250">
					...
					<el-table-column label="图片地址" show-overflow-tooltip="true">
						<el-link slot-scope="scope" type="primary" @click="lookImage(scope.row.imageUrl)">{{ scope.row.imageUrl }}</el-link>
					</el-table-column>
				...
				<el-upload ref="upload" class="upload-demo" action="http://192.168.0.127/ucc/vue/upload" name="fileImage" :multiple="true"
				 limit="6" accept="image/*" :file-list="fileList" :auto-upload="false" list-type="picture" :http-request="uploadFile">
					<el-button slot="trigger" size="small" type="primary">选取需要上传的文件</el-button>
					<el-button style="margin-left: 10px;" size="small" type="success" @click="submitUpload">上传到服务器</el-button>
		...
		methods: {
			...
			editProductFun() {
				...
				this.$confirm('您确定要修改商品的信息吗?', '确认框', {
					...
					var url = "http://192.168.0.143:7778/product/basics/edit/info";
			...
			submitUpload() {
				...
				var url = "http://192.168.0.143:7778/test/testGetNumber";
			...
			editProductSkuKc() {
				this.$confirm('您确定要修改商品SKU库存价格的信息吗?', '确认框', {
					...
					// 发送Ajax删除商品的SPU
					var url = "http://192.168.0.143:7778/product/sku/edit";
					var para = {
			...
			addProductSpuInfo() {
				...
				this.$confirm('您确定要添加商品参数吗?', '确认框', {
					...
					this.COMMON.startLoading();
					// 发送Ajax删除商品的SPU
					var url = "http://192.168.0.143:7778/product/spu/add";
					var para = {
			...
			editSpuInfo() {
				...
				this.$confirm('您确定要修改当前的参数信息吗?', '确认框', {
					...
					this.COMMON.startLoading();
					// 发送Ajax删除商品的SPU
					var url = "http://192.168.0.143:7778/product/spu/edit";
			...
			batchCancelSpu() {
				...
				this.$confirm('是否注销你勾选的商品参数信息!', '确认框', {
					...
					// 发送Ajax删除商品的SPU
					var url = "http://192.168.0.143:7778/product/spu/cancel";
					var para = {
						"spuIds": spuIds
					}
					productAjaxPost(url, para).then(data => {
			...
			normalSpu(index, obj) {
				...
				this.$confirm('是否恢复被删除或者注销的商品SPU信息!', '确认框', {
					...
					this.COMMON.startLoading();
					// 发送Ajax删除商品的SPU
					var url = "http://192.168.0.143:7778/product/spu/normal";
					var para = {
			...
			getProductInfo() {
				// 发送Ajax请求数据
				this.COMMON.startLoading();
				var url = "http://192.168.0.143:7778/product/index";
			...
			shwoProductSpuInfo(id) {
				this.COMMON.startLoading();
				// 发送Ajax获取数据
				var url = "http://192.168.0.143:7778/product/spu/get/" + id;
			...
			removeProducSpu(obj) {
				this.$confirm('是否真的需要删除商品的SPU参数,删除后可在商品SPU信息中恢复!', '确认框', {
					...
					// 发送Ajax删除商品的SPU
					var url = "http://192.168.0.143:7778/product/spu/del";
					var para = {
						"productSpuId": obj.productSpuId
					}
					productAjaxPost(url, para).then(data => {
			...
			showProductSkuInfo(id) {
				...
				this.currentFunctionProductId = id;
				// 发送Ajax查询信息
				var url = "http://192.168.0.143:7778/product/sku/get/" + id;
				productAjaxGet(url).then(data => {
					if (data.status == 200) {
						this.$message({
							type: 'success',
			...
			editProduct(index, id) {
				...
				// 发送AJax查询数据
				var url = "http://192.168.0.143:7778/product/basics/edit/get/" + id;
				productAjaxGet(url).then(data => {
					if (data.status == 200) {
			...
			editProductImageFun() {
				this.COMMON.startLoading()
				// 发送AJax查询数据
				var url = "http://192.168.0.143:7778/product/basics/edit/get/image/" + this.editProductForm.productId;
				productAjaxGet(url).then(data => {
					if (data.status == 200) {
