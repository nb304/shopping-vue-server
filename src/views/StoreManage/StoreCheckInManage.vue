  ...
  <div id="checkInDiv" style="margin:30px ;">
        ...
        <el-table :data="storeCheckInDatas" border style="width: 100%">
          ...
          <el-table-column label="状态" width="100" show-overflow-tooltip="true">
            <template slot-scope="scope">
              ...
              <el-link type="success" v-if="scope.row.storeCheckState == 1">已通过</el-link>
              <el-link type="warning" v-else-if="scope.row.storeCheckState == 2">未通过</el-link>
              <el-link type="info" v-else-if="scope.row.storeCheckState == 3">未处理</el-link>
              <el-link type="danger" v-else>未知状态</el-link>
          ...
          <el-table-column fixed="right" label="操作" min-width="120">
            <template slot-scope="scope">
              <el-button type="text" size="small" :disabled="scope.row.storeCheckState != 3"
                         @click="editStoreState(scope.$index , scope.row , 1)">通过
              </el-button>
              <el-button type="text" :disabled="scope.row.storeCheckState == 1" size="small"
                         @click="openNot(scope.$index , scope.row)">未通过
              </el-button>
            </template>
          </el-table-column>
        </el-table>
    ...
    >
      ...
      <el-row :gutter="24">
        <el-row :gutter="24">
          <el-col :sm="{span: 24}" :xs="{span: 24}">
            <el-input v-if="currentFunctionObj.storeCheckState == 2" :disabled="true"
                      v-model="currentFunctionObj.retain1" placeholder="请输入未通过的理由" type="textarea"
                      style="width: 100%;"/>

            <el-input v-if="currentFunctionObj.storeCheckState == 3"
                      v-model="replayData.content" placeholder="请输入未通过的理由" type="textarea" style="width: 100%;"/>
          </el-col>
        </el-row>
